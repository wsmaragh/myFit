...
class FindVC: UIViewController {
    ...
    private func setupTableView(){
        ...
        self.tableView.frame = CGRect(x: 0, y: 400, width: self.view.bounds.width, height: self.view.bounds.height)
    ...
    private func fetchSearchResults(searchCriteria: SearchCriteria){
        ...
        case .Gym:
            BusinessAPIService.shared.getBusiness(category: .gyms) { (businesses) in
                self.venues = businesses
            }
        case .Parks:
            BusinessAPIService.shared.getBusiness(category: .parks) { (businesses) in
                self.venues = businesses
            }
        case .Events:
            YelpEventAPIService.shared.getActiveEvents { (yelpEvents) in
